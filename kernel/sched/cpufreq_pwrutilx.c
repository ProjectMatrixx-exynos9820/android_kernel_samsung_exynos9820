...
static void pwrgov_work(struct work_struct *work)
{
...
static void pwrgov_irq_work(struct irq_work *irq_work)
{
     ...
     * neglected for now.
     */
    queue_work(sg_policy->workqueue, &sg_policy->work);
}
...
static int pwrgov_kthread_create(struct pwrgov_policy *sg_policy)
{
    ...
    /* Workqueue only required for slow path */
    if (policy->fast_switch_enabled)
        return 0;

    INIT_WORK(&sg_policy->work, pwrgov_work);
    sg_policy->workqueue = create_singlethread_workqueue("pwrgov");
    if (!sg_policy->workqueue) {
        pr_err("failed to create pwrgov workqueue\n");
        return -ENOMEM;
    }
...
static void pwrgov_kthread_stop(struct pwrgov_policy *sg_policy)
{
    ...
    destroy_workqueue(sg_policy->workqueue);
