...
static int dugov_kthread_create(struct dugov_policy *sg_policy)
{
	...
	kthread_init_work(&sg_policy->work, dugov_work);
...
static void dugov_exit(struct cpufreq_policy *policy)
{
...
static void dugov_stop(struct cpufreq_policy *policy)
{
	struct dugov_policy *sg_policy = policy->governor_data;
	unsigned int cpu;
...
static void dugov_limits(struct cpufreq_policy *policy)
{
	struct dugov_policy *sg_policy = policy->governor_data;
